...
function [ac_m,ac_s,nmi_m,nmi_s,ar_m,ar_s] = ...
    CmpExp_DC_MVNMF_L2_warped(options)
...
for inmf = 1:num_nmf
    ...
    tic
    [W,H,Hc,dc_mvnmf_L2_obj] = DC_MVNMF_L2_Norm(fea,options);
    toc
...
% fprintf('dc_mvnmf_l2 the Fscore is : %4.2f+%2.1f\n',mean(Fscore)*100,std2(Fscore)*100);
% fprintf('dc_mvnmf_l2 the Precision is : %4.2f+%2.1f\n',mean(Precision)*100,std2(Precision)*100);
% fprintf('dc_mvnmf_l2 the Recall is : %4.2f+%2.1f\n',mean(Recall)*100,std2(Recall)*100);
