...
for idataname = 1:length(dataset)
    ...
    for idata = 1:5
        ...
        % set paras of DC_MVNMF_L1.
        options = set_paras_dc_mvnmf_l1(dataset{idataname},options); % 'lu' denotes 'label' and 'unlabel'.
        
        % DC_MVNMF_L1
        [ac_m,ac_s,nmi_m,nmi_s,ar_m,ar_s] = CmpExp_DC_MVNMF_L1_warped(options);
        AC(idata) = ac_m;
        NMI(idata) = nmi_m;
        AR(idata) = ar_m;
    end
    rest_DC_MVNMF_L1{1} = [mean(AC)*100,std2(AC)*100];
    rest_DC_MVNMF_L1{2} = [mean(NMI)*100,std2(NMI)*100];
    rest_DC_MVNMF_L1{3} = [mean(AR)*100,std2(AR)*100];
