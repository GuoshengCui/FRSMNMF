function [W,H,Hc,dc_mvnmf_obj] = DC_MVNMF_L2_Norm(X,options)
% modified from "MVRSNMF_16_NoAlpha", auto-weighting strategy is removed.
...
while  iter<=max_iter 
    ...
    %--------------------- update U1 ----------------------%
        ...
        if alpha > 0
            ...
            HcBar = [Y,Hc];
            QHH = Qinv*((HcBar*H{v}').*eye(dim));
            XDH = XDH + alpha*W{v}*QHH;
            Y1 = (H{v}*H{v}').*eye(dim);
            WHDH = WHDH + alpha*W{v}*Y1;
        end
        if beta > 0
            Y2n = (H{v}*Slu{v}*H{v}').*eye(dim);
            XDH = XDH + W{v}*Y2n;
            Y2p = (H{v}*Dlu{v}*H{v}').*eye(dim);
            WHDH = WHDH + W{v}*Y2p;
        end
